...
int main(void){
    double t,dt,r,m,C,v,A,angle,theta,g,v0,pi,w;
    int i,maxstep;
    FILE *fp;
    ...
    fp = fopen("midpoint_wind_-5.dat","w");
    ...
    while(y[i]>0.0 && i<=maxstep){
        ...
        vx[i+1] = vx[i]+dt*ax[i];
        vy[i+1] = vy[i]+dt*ay[i];
        ax[i+1] = -C*v*A*pow(vx[i+1]*vx[i+1]+vy[i+1]*vy[i+1],0.5)*(vx[i+1]-w)/2/m;
