...
int main()
{
  ...
  */
  ...
  fp1 = fopen("t_x_phi_Implicit.dat", "w");
  fp2 = fopen("t_total_psi_Implicit.dat", "w");
  ...
  fp3 = fopen("x_phi_start_Implicit.dat", "w");
  iStep = 0;
  ...
  for (iStep=1; iStep<=nStep; iStep++) {
    ...
    for(i=0;i<=N;i++) diag[i]=(1+2*alpha)+I*(tau/h_bar)*V[i];
    for(i=0;i<=N-1;i++){
      superd[i] = -alpha;
      subd[i] = -alpha;
    }
...
/*
     ...
     for (i=1; i<=N-1; i++) {
        Psi1[i] = Psi0[i]
                  - (I*tau/h_bar)*(-(h_bar*h_bar/(2*mass*h*h))*(Psi0[i+1]+Psi0[i-1]-2*Psi0[i]))
                  + (I*tau/h_bar)*V[i]*Psi0[i];
     }
*/
  ...
  fp3 = fopen("x_phi_end_Implicit.dat", "w");
  for (i=0; i<=N; i++) {
    fprintf( fp3, "%16.8e  %16.8e  %16.8e  %16.8e\n", x[i], pow(cabs(Psi0[i]),2), creal(Psi0[i]), cimag(Psi0[i]) );
  }
