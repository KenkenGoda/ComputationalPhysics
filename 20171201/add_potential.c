...
int main()
{
  ...
  */
  ...
  fp1 = fopen("t_x_phi_potential_5_2.dat", "w");
  fp2 = fopen("t_total_psi_potential_5_2.dat", "w");

  // Initialize the wavefunction
  psi2_total = 0.0;   // sum |Psi^2|

  x0 = -50;           // Location of the center of the wavepacket
  velocity = 1.5;   // Average velocity of the packet
  ...
  fp3 = fopen("x_phi_start_potential_5_2.dat", "w");
  iStep = 0;
  ...
  fp3 = fopen("x_phi_end_potential_5_2.dat", "w");
  for (i=0; i<=N; i++) {
    fprintf( fp3, "%16.8e  %16.8e  %16.8e  %16.8e\n", x[i], pow(cabs(Psi0[i]),2), creal(Psi0[i]), cimag(Psi0[i]) );
  }
