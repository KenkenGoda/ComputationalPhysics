...
int main()
{
  ...
  */
  ...
  double complex Psi0[MAXN], Psi1[MAXN];
  FILE *fp1, *fp2, *fp3;
  ...
  fp1 = fopen("t_x_phi_ftcs.dat", "w");
  fp2 = fopen("t_total_psi_ftcs.dat", "w");
  ...
  fp3 = fopen("x_phi_start_ftcs.dat", "w");
  iStep = 0;
  ...
  fp3 = fopen("x_phi_end_ftcs.dat", "w");
  for (i=0; i<=N; i++) {
    fprintf( fp3, "%16.8e  %16.8e  %16.8e  %16.8e\n", x[i], pow(cabs(Psi0[i]),2), creal(Psi0[i]), cimag(Psi0[i]) );
  }
